#line 1 "/home/terry/Projects/motor_control_firmware/Motor.hpp"
/*! \file Motor.hpp */
...
public:
  ...
  Motor(const char *name, const MotorPin rpwm, const MotorPin lpwm,
        ...
        const MotorPin hall_2, const ControlPin currentSensePin,
  ...
  /// @brief Perform the homing routine to calibrate the position sensor for
  /// this motor
  void home() {
    // First retract as much as possible
    ...
    while (sameCount < 1000) {
      ...
      if (lastPos == pos) {
        if (sameCount == 0) {
          firstSameTime = millis();
        } else {
          if (millis() - firstSameTime > 1000)
            break;
        }
        sameCount++;
      } else {
    ...
    Serial.print("Fully extended. Max pulse: ");
    Serial.println(pos);
    maxPulses = pos;
  ...
  void update(const int newSpeed = (MAX_SPEED + 1)) {
    ...
    /*
      // For lift column - Extension limit
      if (dir == Direction::EXTEND && (pos) > totalPulseCount) {
        dir = Direction::STOP;
        return;
      }

      // For lift column - Retraction limit
      if (dir == Direction::RETRACT && (pos) < 50) {
        dir = Direction::STOP;
        return;
      }
