#line 1 "/home/terry/Projects/motor_control_firmware/PinMacros.hpp"
/*! \file PinMacros.hpp */
...
#define FSET_TO_ANALOG_PIN(pin, var_to_set, range_min, range_max) \
    var_to_set = fmap(analogRead(pin), 0, 4096, range_min, range_max)

#define SET_TO_ANALOG_PIN(pin, var_to_set, range_min, range_max) \
    var_to_set = map(analogRead(pin), 0, 4096, range_min, range_max)

#define SET_TO_ANALOG_PIN_FUNC(pin, func, range_min, range_max) \
func(map(analogRead(pin), 0, 4096, range_min, range_max))
...
#define motorPinMode(pin, value) \
  pinMode(static_cast<std::uint8_t>(pin), value)

#define motorAttachPin(pin, channel) \
  ledcAttachPin(static_cast<std::uint8_t>(pin), channel)

#define motorAnalogRead(pin) \
  analogRead(static_cast<std::uint8_t>(pin))
